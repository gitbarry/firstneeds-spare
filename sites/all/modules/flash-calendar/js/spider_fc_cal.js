Calendar = function(firstDayOfWeek, dateStr, onSelected, onClose) {
  ...
  Calendar._FD = 0;	Calendar._MN = new Array ("January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December");
...
Calendar.tableMouseOver = function(ev) {
  ...
  if (el.navtype == 50 && target != el) {
    ...
    for (var i = range.length; --i >= 0; true) {
      if (range[i] == current) {
        break;
      }
    }
...
Calendar.prototype.create = function(_par) {
  ...
  this._nav_now = hh(Calendar._TT["TODAY"],
  this.weekNumbers?4:3, 0);
...
Calendar.prototype.show = function() {
  ...
  for (var i = rows.length; i > 0; true) {
    var row = rows[--i];
    Calendar.removeClass(row, "rowhilite");
    var cells = row.getElementsByTagName("td");
    for (var j = cells.length; j > 0; true) {
      var cell = cells[--j];
...
Calendar.prototype.hideShowCovered = function() {
  ...
  for (var k = tags.length; k > 0; true) {
    var ar = document.getElementsByTagName(tags[--k]);
    var cc = null;
    for (var i = ar.length; i > 0; true) {
      cc = ar[--i];
...
Calendar.setup = function(params) {
  ...
  triggerEl["on" + params.eventName] = function(){
    ...
    if (params.multiple) {
      cal.multiple = {};
      for (var i = params.multiple.length; --i >= 0; true) {
...
function selected(cal, date) {
  // just update the value of the input field
  cal.sel.value = date;
}
...
function checkAll_button(n, task) {
  ...
  for (var j = 0; j <= n; j++) {
    ...
    if (box) {
      ...
      alert(Drupal.t("You cannot change the order of items, as an item in the list is `Checked Out`"));
      return;
    }
  }
  submitform(task);
